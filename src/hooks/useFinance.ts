...
export const useFinance = () => {
  ...
  const addEntryWithRecurrence = (
    ...
    if (recurrence === 'SINGLE') {
      const entry = createEntry(desc, Money.fromFloat(absVal), type, dateStr);
      newEntries.push({ ...entry, groupId });
    } else {
      for (let i = 0; i < count; i++) {
        ...
        // Cria entrada e adiciona groupId
        const entry = createEntry(
          finalDesc,
          Money.fromFloat(finalValue),
          type,
          currentDate
        );
        newEntries.push({ ...entry, groupId });
  ...
  const kpiData = useMemo(() => {
    ...
    return {
      ...
      ticketMedio:
        ...
        (filteredWorkOrders.filter((o) => o.status === 'FINALIZADO').length || 1),
    };
  ...
  const chartFluxo = useMemo(() => {
    ...
    filteredLedger.forEach((e) => {
      if (filterType === 'DEBIT' && e.type === 'DEBIT') {
        const key =
          viewMode === 'YEAR'
            ? monthLabels[parseInt(e.effectiveDate.slice(5, 7)) - 1]
            : e.effectiveDate.slice(8, 10);
    ...
    let sortedEntries;
    if (viewMode === 'YEAR')
      sortedEntries = Object.entries(map).sort(
        (a, b) => monthLabels.indexOf(a[0]) - monthLabels.indexOf(b[0])
      );
  ...
  const chartPie = useMemo(() => {
    ...
    filteredWorkOrders.forEach((o) => {
      parts += o.parts.reduce((a, i) => a + i.price, 0);
      servs += o.services.reduce((a, i) => a + i.price, 0);
    });
    return parts + servs === 0
      ? [{ name: '-', value: 1 }]
      : [
          { name: 'Peças', value: Money.toFloat(parts) },
          { name: 'Serviços', value: Money.toFloat(servs) },
        ];
  }, [filteredWorkOrders]);
